...
class LessonsService with ReactiveServiceMixin {
  ...
  // Track which courses have already had fix applied to prevent redundant operations
  final Set<String> _fixedCourseIds = {};
  ...
  void _log(String message) {
    if (_enableDebugLogs) {
      debugPrint('LessonsService: $message');
    }
  }
...
// Create a new lesson
  ...
  void updateLessonCompletionStatus(int lessonId, bool isCompleted) {
    _log("Updating lesson $lessonId completion to $isCompleted");
  ...
  Future<void> refreshLessonCompletionStatuses(String courseId) async {
    ...
    for (int i = 0; i < _cachedLessons!.length; i++) {
      ...
      try {
        // Check completion status from the API using standard method
        final isCompleted =
            await completedLessonService.isLessonCompleted(lesson.id);
        _log(
        ...
        _log("Error checking completion status for lesson ${lesson.id}: $e");
      }
    }
  ...
  // Optimized method to fix any inverted completion statuses
  // This is now more efficient and avoids redundant operations
  Future<void> fixInvertedCompletionStatus(String courseId) async {
    if (_cachedLessons == null || _cachedLessons!.isEmpty) return;

    // Skip if we've already fixed this course in this session
    if (_fixedCourseIds.contains(courseId)) {
      _log("Skipping fix for course $courseId - already processed");
      return;
    }

    _log("Checking completion status for course $courseId");
    ...
    try {
      ...
      if (courseLessons.isEmpty) {
        _log("No lessons found for course $courseId");
        return;
      }

      // Refresh completion data once for all lessons
      await completedLessonService.refreshAllCompletionData();

      // Update each lesson's completion status
      int updatedCount = 0;
      for (int i = 0; i < _cachedLessons!.length; i++) {
        final lesson = _cachedLessons![i];
        ...
        try {
          // Check completion status from completion service
          final isCompleted =
              await completedLessonService.isLessonCompleted(lesson.id);

          // Update lesson if needed
          if (lesson.isCompleted != isCompleted) {
            _log("Fixing completion status for lesson ${lesson.id}");
            _cachedLessons![i] = lesson.copyWith(isCompleted: isCompleted);
            updatedCount++;
          }
        } catch (e) {
          _log("Error checking completion for lesson ${lesson.id}: $e");
        }
      }

      if (updatedCount > 0) {
        // Update the reactive value and notify listeners
        _lessons.value = List.from(_cachedLessons!);
        notifyListeners();
        _log("Fixed $updatedCount lesson(s) in course $courseId");
      }

      // Mark this course as fixed
      _fixedCourseIds.add(courseId);
    } catch (e) {
      _log("Error fixing inverted status: $e");
    }
  }
}
